...
export async function getTweets(user) {
    ...
    const request = `http://localhost:7890/1.1/statuses/user_timeline.json?screen_name=${user}&count=${numberOfTweets}`
...
export async function getRetweeters(id) {
    const request = `http://localhost:7890/1.1/statuses/retweeters/ids.json?id=${id}&count=100&stringify_ids=true`
    const response = await apiRequester.send(request)
    const userIds = response.data ? response.data.ids : []
debugger
    const retweeters = await Bluebird.map(userIds, async uid => {
        const request = `http://localhost:7890/1.1/users/show.json?user_id=${uid}`
        const response = await apiRequester.send(request)
